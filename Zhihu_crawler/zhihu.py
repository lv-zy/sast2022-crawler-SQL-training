...
"""
    ...
    def get_board(self) -> list:
        ...
        """
        try:
            # 因为使用hint里面的接口需要登录cookie，所以本次爬虫采用了另外一个热榜接口
            # 另一个页面采取xhr数据传输，获取json数据文件
            req = requests.get('https://www.zhihu.com/api/v4/creators/rank/hot?domain=0&period=hour',headers = self.settings['headers'], timeout=3)  
            data = json.loads(req.text)['data']
            assert data != None
        except:
            logger.info("Cannot get url when getting board")
            return []
        ...
        try:
            ...
            for x in data:
                ...
                heat = reaction['new_pv'] # use increment in place of heat(
                url = question['url']
                qid = question['id']
                excerpt = self.get_question(int(qid))['excerpt']
                row = {
                    'title': title,
                    'heat': heat,
                    'excerpt': excerpt,
            ...
            logger.info('parse data error when gettign board')
            return []
    ...
    def get_question(self, qid: int) -> dict:
        ...
        """
        try:
            # 获取问题页面的html数据，并找到script中包含数据的json，储存在data中
            target_url = 'https://www.zhihu.com/question/' + str(qid)
            req = requests.get(target_url,headers = self.settings['headers'], timeout=1)
            soup = BeautifulSoup(req.text,features='html.parser')
            script = soup.find_all(id = 'js-initialData')[0].text
            data = json.loads(script)['initialState']['entities']['questions'][str(qid)]
        except:
            logger.info('get data error when getting question')
            return {}
        ...
        try:
            ...
            logger.info('parse data error when getting question')
            return {}
